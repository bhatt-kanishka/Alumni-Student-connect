...
public class StudentDashboard extends JFrame {
    ...
    public JPanel buildPanel() {
        ...
        int totalHeight = 980;
        panel.setPreferredSize(new Dimension(CANVAS_W - 20, totalHeight));
        ...
        banner.setBounds(MARGIN, 30, INNER_W - 20, 170);
        banner.setOpaque(false);

        JLabel icon = new JLabel("🎓");
        icon.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 58));
        icon.setBounds(40, 38, 80, 80);
        banner.add(icon);

        JLabel greet = new JLabel("Welcome, " + firstName() + "!");
        greet.setFont(new Font("Georgia", Font.BOLD, 38));
        greet.setForeground(WHITE);
        greet.setBounds(135, 35, 900, 50);
        banner.add(greet);
        ...
        for (int i = 0; i < stats.length; i++) {
            stats[i].setBounds(MARGIN + i * (statW + gap), statY, statW, statH);
            panel.add(stats[i]);
        }

        int row1Y = 380;
        int row1H = 260;
        int leftW = (INNER_W - 20) * 2 / 3 - 15;
        int rightW = (INNER_W - 20) - leftW - gap;

        JPanel mentorCard = makeWhiteCard();
        mentorCard.setBounds(MARGIN, row1Y, leftW, row1H);
        ...
        internCard.setBounds(MARGIN + leftW + gap, row1Y, rightW, row1H);
        ...
        int jy = 60;
        for (String[] job : jobs) {
            internCard.add(buildJobRow(job[0], job[1], job[2], jy, rightW - 35));
            jy += 50;
        }
        ...
        courseCard.setBounds(MARGIN, row2Y, leftW, row2H);
        addSectionTitle(courseCard, "📚 Learning Progress", 24, 18);
        ...
        activityCard.setBounds(MARGIN + leftW + gap, row2Y, rightW, row2H);
        addSectionTitle(activityCard, "🔔 Recent Activity", 24, 18);
        ...
        for (String[] a : activities) {
            activityCard.add(buildActivityRow(a[0], a[1], a[2], ay, rightW - 35));
            ay += 50;
        }
        ...
        actionCard.setBounds(MARGIN, actionY, INNER_W - 20, actionH);
        ...
        int btnW = 260;
        int btnGap = ((INNER_W - 20) - 48 - actions.length * btnW) / (actions.length - 1);
        int ax = 24;

        for (String act : actions) {
            JButton btn = buildQuickActionBtn(act);
            btn.setBounds(ax, 48, btnW, 42);
        ...
        panel.setPreferredSize(new Dimension(CANVAS_W - 20, actionY + actionH + 40));
    ...
    private JPanel buildStatCard(String title, String value, String sub, Color accent, String icon) {
        ...
        ic.setBounds(22, 18, 45, 40);
        card.add(ic);

        JLabel ttl = new JLabel(title);
        ttl.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        ttl.setForeground(LABEL_LIGHT);
        ttl.setBounds(22, 44, 200, 20);
        card.add(ttl);

        JLabel val = new JLabel(value);
        val.setFont(new Font("Georgia", Font.BOLD, 32));
        val.setForeground(LABEL_DARK);
        val.setBounds(22, 65, 160, 35);
        card.add(val);

        JLabel sb = new JLabel(sub);
        sb.setFont(new Font("Segoe UI", Font.BOLD, 13));
        sb.setForeground(accent);
        sb.setBounds(22, 98, 220, 18);
        card.add(sb);
    ...
    private JPanel buildMentorRow(String initials, String name, String role, String skills, Color color, int y, int w) {
        ...
        row.setOpaque(false);
        row.setBounds(18, y, w, 64);

        JLabel av = avatar(initials, color, 48);
        av.setBounds(0, 8, 48, 48);
        row.add(av);

        JLabel nm = new JLabel(name);
        nm.setFont(new Font("Segoe UI", Font.BOLD, 16));
        nm.setForeground(LABEL_DARK);
        nm.setBounds(65, 6, 300, 24);
        row.add(nm);

        JLabel info = new JLabel(role + " · " + skills);
        info.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        info.setForeground(LABEL_LIGHT);
        info.setBounds(65, 30, 400, 20);
        row.add(info);

        JButton connect = buildActionBtn("Connect", color);
        connect.setBounds(w - 115, 16, 100, 30);
        row.add(connect);
